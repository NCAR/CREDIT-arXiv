...
energy from IFS outputs. It runs with config file `verif_config_6h.yml` with
hard coded forecast lead time of [24, 120, 240] hours. 

The script calculates energy spectrum on a given range of initializations:
```
python STEP03_ZES_IFS_6h.py 0 365
```
...
for i, ind_pick in enumerate(ind_lead):
    ...
    for fn_ours in filename_OURS:
        ...
        ds_ours = ds_ours.rename({'lat':'latitude', 'lon':'longitude'})
